...
export interface NetworkState {
  ...
  registrars: Partial<Record<NetworkId, RegistrarRecord>>;
}

interface NetworkStateV1 {
  version: 1;
  activeNetwork: NetworkId;
  wallets: Partial<Record<NetworkId, { seed: string; createdAt: string }>>;
  deployments: Partial<Record<NetworkId, DeploymentRecord>>;
...
export function loadState(opts: FsOptions = {}): NetworkState | null {
  ...
  if (!parsed || typeof parsed !== 'object') {
    throw new Error(`Unsupported state-file contents in ${p}. Preserve the file and migrate explicitly.`);
  }
  const version = (parsed as { version?: unknown }).version;
  if (version !== 1 && version !== STATE_VERSION) {
    throw new Error(
      `Unsupported state-file version in ${p} (expected 1 or ${STATE_VERSION}). Preserve the file and migrate explicitly.`,
    );
  }
  ...
  if (version === 1) {
    const legacy = parsed as NetworkStateV1;
    return {
      ...legacy,
...
function registrarEnvName(network: NetworkId): string {
  return `PROOFRAIL_REGISTRAR_SECRET_${network.toUpperCase()}`;
}

function assertSecret(value: string, source: string): string {
  const normalized = value.trim().toLowerCase();
  if (!/^[0-9a-f]{64}$/.test(normalized)) {
    throw new Error(`${source} must contain exactly 32 bytes encoded as hexadecimal.`);
  }
  return normalized;
}

export function getOrCreateRegistrarSecret(
  network: NetworkId,
  opts: SeedOptions = {},
): string {
  const env = opts.env ?? process.env;
  const cwd = opts.cwd ?? process.cwd();
  const envName = registrarEnvName(network);
  const fromEnv = env[envName];
  if (fromEnv) return assertSecret(fromEnv, envName);

  const existing = loadState({ cwd });
  const persisted = existing?.registrars?.[network]?.secret;
  if (persisted) return assertSecret(persisted, `${STATE_FILE_NAME} registrar`);
  ...
  next.registrars = {
    ...next.registrars,
    [network]: { secret, createdAt: now, rotatedAt: null },
  };
  saveState(next, { cwd });
  return secret;
}

export function recordRegistrarRotation(
  network: NetworkId,
  secret: string,
  opts: SeedOptions = {},
): void {
  ...
  if (env[envName]) {
    throw new Error(
      `${envName} is externally managed. Update that secret source after the on-chain rotation.`,
    );
  }
  const normalized = assertSecret(secret, 'Rotated registrar secret');
  const existing = loadState({ cwd });
  if (!existing) throw new Error(`${STATE_FILE_NAME} is missing.`);
  const previous = existing.registrars?.[network];
  existing.registrars = {
    ...existing.registrars,
    [network]: {
      secret: normalized,
