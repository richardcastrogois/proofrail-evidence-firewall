/**
 * Deploy midnight-chain contract to a Midnight network (undeployed by default; use --network preview|preprod for public networks).
 *
...
function isTransientSubmissionError(err: unknown): boolean {
  const text = err instanceof Error
    ? `${err.name} ${err.message} ${(err as Error & { cause?: unknown }).cause ?? ''}`
    : String(err);
  return (
    text.includes('SubmissionError') ||
    text.includes('submitAndWatchExtrinsic') ||
    text.includes('Normal Closure') ||
    text.includes('disconnected from') ||
    text.includes('Transaction submission')
  );
}

async function registerDustWithRetry(walletCtx: WalletContext): Promise<void> {
  const MAX_ATTEMPTS = 8;
  const BASE_DELAY_MS = 10_000;
  const DUST_WAIT_TIMEOUT_MS = 300_000;
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    ...
    try {
      const registrationEstimate = await walletCtx.wallet.estimateRegistration(unregisteredUtxos);
      const registrationFee = registrationEstimate.fee ?? 0n;
      if (registrationFee > 0n) {
        console.log(`  Waiting for generated DUST to cover registration fee: ${registrationFee.toLocaleString()}`);
      }
      await walletCtx.wallet.waitForGeneratedDust(
        unregisteredUtxos,
        registrationFee,
        { timeoutMs: DUST_WAIT_TIMEOUT_MS },
      );

      // The signDustRegistration callback already produces a recipe with the
      // required input signatures. Do not sign the recipe again.
      ...
      if (!isTransientSubmissionError(err) || attempt === MAX_ATTEMPTS) {
        throw err;
      }
      const delayMs = Math.min(BASE_DELAY_MS * attempt, 60_000);
      const message = err instanceof Error ? err.message : String(err);
      console.log(`  Registration submit failed transiently: ${message}`);
      console.log(`  Retrying DUST registration in ${Math.round(delayMs / 1000)}s...`);
      await new Promise((r) => setTimeout(r, delayMs));
    }
  }
}
...
async function main() {
  console.log('\n╔══════════════════════════════════════════════════════════════╗');
  console.log(`║  Deploy midnight-chain to ${network}`);
  console.log('╚══════════════════════════════════════════════════════════════╝\n');
  ...
  await registerDustWithRetry(walletCtx);
